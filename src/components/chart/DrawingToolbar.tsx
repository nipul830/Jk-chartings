...
export function DrawingToolbar({chart,symbol,timeframe,chartId}:Props){
 ...
 const point=(e:{clientX:number;clientY:number})=>{if(!chart||!box.current)return null;const r=box.current.getBoundingClientRect();const x=Math.max(0,Math.min(r.width,e.clientX-r.left)),y=Math.max(0,Math.min(r.height,e.clientY-r.top));const time=chart.timeScale().coordinateToTime(x);const price=chart.timeScale().coordinateToLogical(x);if(time==null||price==null)return null;const seriesPrice=chart.priceScale("right").height();const range=chart.priceScale("right").getVisiblePriceRange();if(!range||!seriesPrice)return null;const p=range.min+(range.max-range.min)*(y/seriesPrice);return{time:Number(time),price:p}};
 const pixel=(p:Anchor)=>{if(!chart)return null;const x=chart.timeScale().timeToCoordinate(p.time as any);const range=chart.priceScale("right").getVisiblePriceRange();const h=chart.priceScale("right").height();if(x==null||!range||!h)return null;const y=(range.max-p.price)/(range.max-range.min)*h;return{x,y}};
