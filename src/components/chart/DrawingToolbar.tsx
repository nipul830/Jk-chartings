...
export function DrawingToolbar({chart,symbol,timeframe,chartId}:Props){
 ...
 const point=(e:{clientX:number;clientY:number})=>{if(!chart||!box.current)return null;const r=box.current.getBoundingClientRect();const x=Math.max(0,Math.min(r.width,e.clientX-r.left)),y=Math.max(0,Math.min(r.height,e.clientY-r.top));const time=chart.timeScale().coordinateToTime(x);const logical=chart.timeScale().coordinateToLogical(x);if(time==null||logical==null)return null;const range=chart.priceScale("right").getVisiblePriceRange();const h=r.height;if(!range||!h)return null;const p=range.min+(range.max-range.min)*(y/h);return{time:Number(time),price:p}};
 const pixel=(p:Anchor)=>{if(!chart||!box.current)return null;const x=chart.timeScale().timeToCoordinate(p.time as any),range=chart.priceScale("right").getVisiblePriceRange(),h=box.current.clientHeight;if(x==null||!range||!h||range.max===range.min)return null;const y=(range.max-p.price)/(range.max-range.min)*h;return{x,y}};
