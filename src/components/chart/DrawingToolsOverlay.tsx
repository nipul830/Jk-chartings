...
// Drawings are a Lightweight Charts series primitive, so candles and drawings are
// painted by the same chart renderer. This removes the React/SVG frame lag during pan/zoom.
...
export function DrawingToolsOverlay({chart,series,height}:Props){
 const host=useRef<HTMLDivElement>(null);const primitive=useRef<DrawingPrimitive|null>(null);const[open,setOpen]=useState(false);const[active,setActive]=useState("cross");const[drawings,setDrawings]=useState<Drawing[]>([]);const[draft,setDraft]=useState<Point[]>([]);const[cursor,setCursor]=useState<Point|null>(null);const[color,setColor]=useState("#f5f5f5");const[width,setWidth]=useState(2);const drawing=active!=="cross";
 ...
 useEffect(()=>{const s=series.current;if(!s)return;const p=new DrawingPrimitive();primitive.current=p;s.attachPrimitive(p as any);return()=>{try{s.detachPrimitive(p as any)}catch{}primitive.current=null}},[series]);
