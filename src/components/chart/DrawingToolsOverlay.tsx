...
export function DrawingToolsOverlay({chart,series,height}:Props){
 ...
 const panRef=useRef({active:false,lastX:0,lastY:0,dx:0,dy:0});
 const[open,setOpen]=useState(false);const[active,setActive]=useState("cross");const[drawings,setDrawings]=useState<Drawing[]>([]);const[draft,setDraft]=useState<Point[]>([]);const[cursor,setCursor]=useState<Point|null>(null);const[color,setColor]=useState("#f5f5f5");const[width,setWidth]=useState(2);const[,redraw]=useState(0);const drawing=active!=="cross";
 ...
 // During chart panning, move the drawing SVG immediately with the same pointer delta.
 // This avoids waiting for React/chart subscription callbacks, which can produce a visible one-frame lag.
 useEffect(()=>{
   if(drawing)return;
   ...
   const setTransform=()=>{svg.style.transform=`translate3d(${panRef.current.dx}px,${panRef.current.dy}px,0)`};
   const start=(e:PointerEvent)=>{if(e.button!==0||!inside(e.target))return;panRef.current={active:true,lastX:e.clientX,lastY:e.clientY,dx:0,dy:0};};
   const move=(e:PointerEvent)=>{const p=panRef.current;if(!p.active||!inside(e.target))return;p.dx+=e.clientX-p.lastX;p.dy+=e.clientY-p.lastY;p.lastX=e.clientX;p.lastY=e.clientY;setTransform();};
   const end=()=>{const p=panRef.current;if(!p.active)return;p.active=false;p.dx=0;p.dy=0;p.lastX=0;p.lastY=0;svg.style.transform="translate3d(0,0,0)";redraw(v=>v+1);};
   window.addEventListener("pointerdown",start,true);window.addEventListener("pointermove",move,true);window.addEventListener("pointerup",end,true);window.addEventListener("pointercancel",end,true);
   return()=>{window.removeEventListener("pointerdown",start,true);window.removeEventListener("pointermove",move,true);window.removeEventListener("pointerup",end,true);window.removeEventListener("pointercancel",end,true);svg.style.transform="translate3d(0,0,0)"};
 },[chart,drawing]);

 // Reconcile exact candle/time/price positions after the chart finishes a viewport update.
 useEffect(()=>{
   const c=chart.current;if(!c)return;
   const refresh=()=>{if(!panRef.current.active)redraw(v=>v+1)};
   const move=(p:MouseEventParams)=>{if(!p.point)return;const s=series.current;if(!s)return;const y=Math.max(0,Math.min(height,p.point.y));const price=s.coordinateToPrice(y);const t=c.timeScale().coordinateToTime(p.point.x);setCursor({x:p.point.x,y,time:typeof t==="number"?t:undefined,price:price===null?undefined:price});if(!panRef.current.active)refresh()};
   c.subscribeCrosshairMove(move);c.timeScale().subscribeVisibleLogicalRangeChange(refresh);window.addEventListener("resize",refresh);
   return()=>{c.unsubscribeCrosshairMove(move);c.timeScale().unsubscribeVisibleLogicalRangeChange(refresh);window.removeEventListener("resize",refresh)};
 },[chart,series,height]);
 ...
 const xy=(p:Point)=>{const c=chart.current,s=series.current;if(!c||!s)return{x:p.x,y:p.y};let x=p.x,y=p.y;if(typeof p.time==="number"){const v=c.timeScale().timeToCoordinate(p.time as any);if(v!==null)x=v}if(typeof p.price==="number"){const v=s.priceToCoordinate(p.price);if(v!==null)y=v}return{x,y}};
