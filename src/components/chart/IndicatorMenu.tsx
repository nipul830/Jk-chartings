...
export function IndicatorMenu(){
 ...
 return <div className="relative">
  ...
  {open&&<div className="absolute right-0 top-11 z-[200] w-[min(360px,calc(100vw-24px))] rounded-xl border border-[#333] bg-[#080808] shadow-2xl overflow-hidden">
   <div className="flex items-center justify-between px-4 py-3 border-b border-[#222]"><div><div className="font-medium">Indicators</div><div className="text-[11px] text-[#666]">Add indicators to this chart</div></div><button onClick={()=>setOpen(false)} className="text-[#888] hover:text-white"><X size={17}/></button></div>
   <div className="max-h-[55vh] overflow-y-auto p-2">{INDICATORS.map(name=><div key={name} className="flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-[#111]"><button onClick={()=>toggle(name)} className="flex-1 flex items-center gap-3 text-left text-sm"><span className={`w-5 h-5 rounded border flex items-center justify-center ${selected.includes(name)?"bg-white text-black border-white":"border-[#444] text-transparent"}`}><Check size={13}/></span><span>{name}</span></button>{selected.includes(name)&&<button title="Edit indicator" onClick={()=>startEdit(name)} className="w-8 h-8 flex items-center justify-center rounded border border-[#333] text-[#aaa] hover:text-white hover:border-white"><Settings2 size={15}/></button>}</div>)}</div>
   {edit&&<div className="border-t border-[#222] p-4 bg-[#0c0c0c]"><div className="flex items-center justify-between mb-3"><div className="font-medium text-sm">Edit {edit}</div><button onClick={()=>setEdit(null)} className="text-[#777]"><X size={15}/></button></div><div className="grid grid-cols-2 gap-2">{Object.entries(cfg).map(([key,value])=><label key={key} className="text-[11px] text-[#777] capitalize">{key}<input type="number" value={value} onChange={e=>setCfg(x=>({...x,[key]:Number(e.target.value)}))} className="mt-1 w-full rounded border border-[#333] bg-black px-2 py-2 text-sm text-white outline-none focus:border-white"/></label>)}</div><button onClick={save} className="w-full mt-3 rounded-md bg-white text-black py-2 text-sm font-medium">Apply</button></div>}
  </div>}
 </div>;
}
