...
export function ChartPane({symbol,timeframe,height=400,onSymbolClick,onTimeframeChange}:Props){
 ...
 useEffect(()=>{const tick=()=>{const s=seconds[timeframe]||900;const now=Math.floor(Date.now()/1000);const rem=s-(now%s);setCountdown(rem===s?0:rem);if(latestPrice!==null&&chart.current){try{const size=chart.current.getSize("candle_pane","main");const y=chart.current.convertFromPixel?chart.current.convertFromPixel({x:size.width-30,y:size.height/2},"candle_pane")?.value:null;if(typeof y==="number")setCountdownTop(Math.max(4,Math.min(height-24,size.height/2)))}catch{}}};tick();const id=window.setInterval(tick,250);return()=>window.clearInterval(id)},[timeframe,latestPrice,height]);
 useEffect(()=>{if(!el.current)return;let disposed=false;const cc=colors();const light=document.documentElement.classList.contains("light");const c=init(el.current,{styles:light?"light":"dark",layout:{pane:{height,minHeight:120},yAxis:{position:"right"}}} as any);if(!c)return;chart.current=c;
 c.setStyles({candle:{type:"candle_solid",bar:{upColor:cc.up,downColor:cc.down,upBorderColor:cc.up,downBorderColor:cc.down,upWickColor:cc.up,downWickColor:cc.down}}});
 ...
 return()=>{disposed=true;mo.disconnect();ro.disconnect();window.removeEventListener("jk-candle-colors-changed",theme);try{dispose(c)}catch{};chart.current=null};
 },[height]);
 useEffect(()=>{let stop=false;const load=async()=>{if(!chart.current)return;try{const r=await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${intervals[timeframe]||"15m"}&limit=1000`,{cache:"no-store"});const raw=await r.json();if(stop||!Array.isArray(raw)||!raw.length)return;const d:Candle[]=raw.map((k:any)=>({time:Math.floor(k[0]/1000),open:+k[1],high:+k[2],low:+k[3],close:+k[4],volume:+k[5]}));const last=d[d.length-1];setLatestPrice(last.close);const data=d.map(x=>({timestamp:x.time*1000,open:x.open,high:x.high,low:x.low,close:x.close,volume:x.volume,turnover:((x.open+x.high+x.low+x.close)/4)*x.volume}));
 const cfgs=readConfig();const c=chart.current;try{c.applyNewData(data);c.scrollToRealTime()}catch(e){console.error(e)}
 for(const item of indicators){const cfg:any=cfgs[item.id]||cfgs[item.name]||{};const result:any=calculateIndicator(item.name,d,cfg);if(!result)continue;
   const customName=`JK_${safe(item.id)}_${safe(item.name)}`;
