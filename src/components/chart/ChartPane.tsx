...
export function ChartPane({symbol,timeframe,height=400,onSymbolClick}:ChartPaneProps){
  ...
  const handlePointerDown=(e:PointerEvent<HTMLDivElement>)=>{if(!isDrawTool||showTools||showFibSettings)return;e.preventDefault();const p=pointFromEvent(e);if(selectedTool==="Horizontal line"||selectedTool==="Vertical line"){setDrawings(d=>[...d,{tool:selectedTool,a:p,b:p}]);return;}setDraftStart(p);setDraftEnd(p);e.currentTarget.setPointerCapture?.(e.pointerId);};
  ...
  const renderDrawing=(d:Drawing,i:number)=>{
    const {a,b,tool}=d; if(tool==="Horizontal line")return <line key={i} x1="0" y1={a.y} x2="100%" y2={a.y} stroke="currentColor" strokeWidth="1.2"/>;
    ...
    if(tool==="Ray"){const vx=b.x-a.x,vy=b.y-a.y;const t=Math.max(1,(Math.max(containerRef.current?.clientWidth||1,containerRef.current?.clientHeight||1)*2)/Math.max(Math.abs(vx),Math.abs(vy),1));return <line key={i} x1={a.x} y1={a.y} x2={a.x+vx*t} y2={a.y+vy*t} stroke="currentColor" strokeWidth="1.2"/>;}
  ...
  return <div className="w-full h-full relative border border-[#1a1a1a] bg-black light:bg-white">
    ...
    {draftStart&&<div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 rounded bg-black/80 border border-[#333] px-2 py-1 text-[11px] text-white pointer-events-none">Tap second point to place {selectedTool}</div>}
