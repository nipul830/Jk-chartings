...
export function ChartPane({symbol,timeframe,height=400,onSymbolClick,onTimeframeChange}:Props){
 ...
 useEffect(()=>{if(!el.current)return;let disposed=false;const cc=colors();const light=document.documentElement.classList.contains("light");const c=init(el.current,{styles:light?"light":"dark",layout:{pane:{height,minHeight:120},yAxis:{position:"right"}}} as any);chart.current=c;
