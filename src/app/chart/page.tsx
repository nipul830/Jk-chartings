...
function ChartContent(){
 ...
 // Keep every pane inside the available viewport so 4/6 layouts never create internal scrollbars.
 const chartHeight=layout===1?600:layout===2?560:layout===4?280:280;
 return <div className="h-[calc(100vh-56px)] min-h-0 flex flex-col bg-black overflow-hidden">
  ...
  <div className={`flex-1 min-h-0 grid ${gridClass} gap-px p-px overflow-hidden bg-[#222]`}>{charts.map((chart,i)=><div key={chart.id} onClick={()=>setActive(i)} className={`min-h-0 min-w-0 overflow-hidden bg-black ${active===i?"ring-1 ring-inset ring-[#555]":""}`}><ChartPane key={`${chart.id}-${chart.symbol}-${chart.timeframe}`} symbol={chart.symbol} timeframe={chart.timeframe} height={chartHeight} onSymbolClick={()=>setSymbolPicker(chart.id)} onTimeframeChange={tf=>updateChart(chart.id,{timeframe:tf})}/></div>)}</div>
