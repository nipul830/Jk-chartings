...
function ChartContent(){
 ...
 const height=layout===1?600:layout===2?450:layout===4?Math.max(280,window.innerHeight/2-85):Math.max(240,window.innerHeight/2-85);
 return <div className="h-[calc(100vh-56px)] flex flex-col bg-black">
  <div className="border-b border-[#222] bg-[#080808]"><div className="flex items-center gap-2 px-2 py-2 overflow-x-auto whitespace-nowrap">
   ...
   <TimeframeSelector value={activeChart?.timeframe||"15m"} onChange={tf=>charts.forEach(c=>updateChart(c.id,{timeframe:tf}))}/><IndicatorMenu/><div className="h-6 w-px bg-[#333]"/><LayoutSelector value={layout} onChange={setLayout}/>
  ...
  <div className={`flex-1 min-h-0 grid ${gridClass} gap-px p-px overflow-hidden bg-[#222]`}>{charts.map((chart,i)=><div key={chart.id} onClick={()=>setActive(i)} className={`min-h-0 min-w-0 bg-black ${active===i?"ring-1 ring-inset ring-[#555]":""}`}><ChartPane key={`${chart.id}-${chart.symbol}-${chart.timeframe}`} symbol={chart.symbol} timeframe={chart.timeframe} height={height} onSymbolClick={()=>setSymbolPicker(chart.id)}/></div>)}</div>
