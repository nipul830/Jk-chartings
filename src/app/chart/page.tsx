...
function ChartContent(){
 ...
 const gridClass=layout===1?"grid-cols-1":layout===2?"grid-cols-1 md:grid-cols-2":"grid-cols-1 md:grid-cols-2 lg:grid-cols-3";
 ...
 return <div className="h-[calc(100vh-56px)] flex flex-col bg-black">
  ...
  <div className={`flex-1 grid ${gridClass} gap-px p-px overflow-auto bg-[#222]`}>{charts.map((chart,i)=><div key={chart.id} onClick={()=>setActive(i)} className={`min-h-[280px] bg-black ${active===i?"ring-1 ring-inset ring-[#555]":""}`}><ChartPane key={`${chart.id}-${chart.symbol}-${chart.timeframe}`} symbol={chart.symbol} timeframe={chart.timeframe} height={height} onSymbolClick={()=>setSymbolPicker(chart.id)}/></div>)}</div>
