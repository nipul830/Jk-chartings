      ...
      <section className="border-b border-[#222] bg-black">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-center px-4 sm:px-6">
          <div className="flex items-center gap-2 text-2xl font-medium">
            <span>$0.00</span>
            <span className="text-xl text-[#888]">0.00</span>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-3 border-b border-[#222]">
          {tabs.map((item) => (
            <button key={item} type="button" onClick={() => setTab(item)}
              className={`relative py-5 text-center text-base sm:text-lg ${tab === item ? "font-semibold text-white" : "text-[#777]"}`}>
              {item}
        ...
        <div className="flex min-h-[calc(100vh-180px)] flex-col items-center justify-center px-6 py-20">
          {tab === "Position" && orders.length > 0 ? (
            <div className="w-full max-w-xl space-y-3">
              ...
              <button type="button" onClick={() => setOpenOrder(true)} className="mx-auto block rounded-lg border border-[#444] bg-white px-8 py-3 text-base font-medium text-black transition hover:bg-[#ddd]">Open position</button>
            </div>
          ) : (
            <>
              <div className="mb-5 flex h-28 w-28 items-center justify-center text-[#333]"><FileText size={96} strokeWidth={1.2} /></div>
              <p className="mb-10 text-xl text-[#777]">
                {tab === "Position" ? "Currently no position" : tab === "Pending Order" ? "Currently no pending order" : "Currently no closed position"}
              </p>
              {tab === "Position" && (
                <button type="button" onClick={() => setOpenOrder(true)} className="rounded-lg border border-[#444] bg-white px-8 py-3 text-base font-medium text-black shadow-sm transition hover:bg-[#ddd]">Open position</button>
              )}
            </>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 sm:items-center">
          <div className="w-full max-w-md rounded-t-2xl border border-[#292929] bg-[#0b0b0b] p-5 sm:rounded-2xl">
            <div className="mb-5 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpenOrder(false)} aria-label="Close" className="text-[#888] hover:text-white"><X /></button>
            </div>
            ...
            <label className="mb-3 block text-sm text-[#aaa]">Symbol
              <input value={symbol} onChange={(e) => setSymbol(e.target.value)} className="mt-1 w-full rounded-lg border border-[#333] bg-black px-3 py-3 text-white outline-none focus:border-[#777]" />
            </label>
            <label className="mb-5 block text-sm text-[#aaa]">Quantity
              <input type="number" min="0.000001" step="any" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="mt-1 w-full rounded-lg border border-[#333] bg-black px-3 py-3 text-white outline-none focus:border-[#777]" />
            </label>
