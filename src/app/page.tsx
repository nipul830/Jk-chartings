...
export default function HomePage() {
  ...
  const { symbols, addSymbol, removeSymbol, setSymbols } = useWatchlistStore();
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 bg-white text-black text-sm font-medium rounded hover:bg-gray-200 transition"
        >
          <Plus size={16} />
          Add Symbol
        </button>
            ...
            <div className="max-h-60 overflow-y-auto">
              {filtered.map((sym) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 hover:bg-[#111] rounded text-sm flex justify-between"
                >
                  <span>{sym}</span>
              ...
              {filtered.length === 0 && (
                <p className="text-[#666] text-sm text-center py-4">No symbols found</p>
              )}
            </div>
            <button
              onClick={() => setShowSearch(false)}
              className="mt-3 w-full py-2 border border-[#333] rounded text-sm hover:bg-[#111]"
            >
              Close
            </button>
          </div>
      ...
      <div className="border border-[#222] rounded-lg overflow-hidden">
        ...
        {symbols.map((sym) => {
          ...
          return (
            <div
              key={sym}
              onClick={() => router.push(`/chart?symbol=${sym}`)}
              className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-[#1a1a1a] hover:bg-[#0a0a0a] cursor-pointer transition-colors group"
            >
              <div className="col-span-5 font-medium flex items-center gap-2">
                {sym.replace("USDT", "")}
                <span className="text-[#555] text-xs">/USDT</span>
              </div>
              <div className="col-span-4 text-right font-mono">
                {data ? data.price : "—"}
              </div>
              <div
                className={`col-span-3 text-right font-mono flex items-center justify-end gap-1 ${
                  isPositive ? "text-white" : "text-[#888]"
                }`}
              >
        ...
        {symbols.length === 0 && (
          <div className="py-12 text-center text-[#555]">
            No symbols in watchlist. Click “Add Symbol”.
          </div>
        )}
      </div>
    </div>
